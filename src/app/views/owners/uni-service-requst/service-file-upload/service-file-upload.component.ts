...
response2;
  ...
  upload()
  {
    this.http.post('http://localhost:58989/api/ServiceFileUpload', this.response, {reportProgress: true, observe: 'events'})
    .subscribe(event2 => {
    ...
    this.http.post('http://localhost:58989/api/ServiceFileUpload', this.response1, {reportProgress: true, observe: 'events'})
    .subscribe(event2 => {
      if (event2.type === HttpEventType.UploadProgress)
